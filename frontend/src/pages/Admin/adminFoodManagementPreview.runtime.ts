...
import {
  ...
  escapeHtml,
  extractIsoDate,
  foodManagementHeroButtonConfig,
  formatMoney,
  formatUkDate,
  formatUkDateInput,
  getCodeStatusMeta,
  ...
  packageDescriptionFallbacks,
  parseUkDateInput,
  type PackageDraftRow,
  type ScopedFoodBankOption,
...
import { safeReferenceScript } from './adminFoodManagementPreview.source'
...
interface SyncAdminFoodManagementPreviewOptions {
...
export function syncAdminFoodManagementPreviewIframe({
  iframe,
  navigate,
  adminScope,
  accessToken,
  ...
  logout,
  setLoginModal,
}: SyncAdminFoodManagementPreviewOptions) {
  const isLocalFoodBankAdmin = adminScope.isLocalFoodBankAdmin
  ...
  const syncIframe = () => {
      ...
      ensureDocumentStyle(
        doc,
        'food-management-runtime-overrides',
        `
          ${ADMIN_PREVIEW_HEADER_GRID_OVERRIDES}
          ...
          .admin-scope-filter-field .filter-select {
            width: 100%;
          }
      ...
      const ensureFilterCluster = (
        ...
        let cluster = doc.getElementById(clusterId) as HTMLElement | null
        if (!cluster) {
          cluster = doc.createElement('div')
          cluster.id = clusterId
          cluster.className = 'admin-scope-filter-cluster'
          actions.appendChild(cluster)
        }
        return cluster
      }

      const ensureScopedFilterSelect = (
        ...
        let field = doc.getElementById(`${fieldId}-field`) as HTMLElement | null
        if (!field) {
          field = doc.createElement('div')
          field.id = `${fieldId}-field`
          field.className = 'admin-scope-filter-field'
          cluster.appendChild(field)
        }
        field.querySelector('.admin-scope-filter-label')?.remove()

        let select = doc.getElementById(fieldId) as HTMLSelectElement | null
        if (!select) {
          select = doc.createElement('select')
          select.id = fieldId
          select.className = 'filter-select'
          field.appendChild(select)
        } else if (!field.contains(select)) {
          field.appendChild(select)
        }
      ...
      if (!isLocalFoodBankAdmin) {
        ...
        inventoryFoodBankFilter = ensureScopedFilterSelect(
          inventoryCluster,
          'inventory-food-bank-filter',
        )
        if (existingInventoryCategoryFilter) {
      ...
      if (isAuthenticated && accessToken) {
        ...
        const inventoryCategoryFilter = doc.getElementById(
        ...
        const getInventoryFilterCategories = (): string[] => {
          const targetFoodBankId = getTargetCreationFoodBankId()
          const sourceItems = isLocalFoodBankAdmin
            ? inventoryItems
            : getScopedInventoryItems(targetFoodBankId)

          return Array.from(new Set(sourceItems.map((item) => item.category)))
            .filter((category) =>
              inventoryCategoryOptions.includes(category as (typeof inventoryCategoryOptions)[number]),
            )
        ...
        const syncScopedFilterSelections = () => {
          ...
          const setFoodBankOptions = (select: HTMLSelectElement | null) => {
            if (!select) {
              return
            }
            ...
            select.innerHTML = `
              ...
              ${availableFoodBanks
                ...
                .join('')}
            `
            select.value = selectedFoodBankValue
          }
          ...
          if (inventoryCategoryFilter) {
            const categories = getInventoryFilterCategories()
            const previousValue = inventoryCategoryFilter.value
            inventoryCategoryFilter.hidden = false
            inventoryCategoryFilter.style.display = 'block'
            inventoryCategoryFilter.innerHTML = `
              ...
              ${categories
                ...
                .join('')}
            `
            inventoryCategoryFilter.disabled = false
          ...
          if (packageSearchInput) {
            packageSearchInput.disabled = !selectedFoodBankId && !isLocalFoodBankAdmin
        ...
        const applyScopedFoodBankSelection = (foodBankId: number | null) => {
          packageBankId = isValidScopedFoodBankId(foodBankId) ? Number(foodBankId) : null
          ...
          if (newPackageEditor?.classList.contains('visible')) {
            setPackageItemRows(
              newPackageEditor,
        ...
        const ensureCreationFoodBankField = (
          ...
          select.onchange = () => {
            ...
            applyScopedFoodBankSelection(
              isValidScopedFoodBankId(nextFoodBankId) ? nextFoodBankId : null,
            )
          ...
          if (filteredPackages.length === 0) {
            packageGrid.innerHTML = `
              <div class="card">
                ...
                <p style="color: var(--color-text-light); font-size: 13px; margin-bottom: var(--spacing-sm);">${escapeHtml(
                  isLocalFoodBankAdmin
                    ? 'No food packages are currently assigned to this food bank.'
                    : `No food packages match ${getFoodBankLabel(selectedFoodBankId)}. Adjust your search or create a new package in this scope.`,
                )}</p>
              </div>
            `
            return
        ...
        const getFilteredInventoryItems = (): AdminInventoryItemRecord[] => {
          ...
          return inventoryItems.filter((item) => {
            if (!isLocalFoodBankAdmin) {
              ...
              if (item.food_bank_id !== selectedFoodBankId) {
                return false
              }
            }
            if (categoryFilter && item.category !== categoryFilter) {
        ...
        const bindScopedFoodBankFilter = (select: HTMLSelectElement | null) => {
          ...
          const handleScopedFoodBankChange = () => {
            const nextFoodBankId = Number(select.value || '0')
            applyScopedFoodBankSelection(
              isValidScopedFoodBankId(nextFoodBankId) ? nextFoodBankId : null,
            )
          ...
          select.addEventListener('change', handleScopedFoodBankChange)
