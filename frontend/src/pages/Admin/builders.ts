...
import {
  ...
  type DonationEditorDraft,
  type DonationEditorItemDraft,
  type InventoryEditorDraft,
  type InventoryStockInDraft,
  type PackageEditorDraft,
  type PackageEditorRowDraft,
  type PackageRow,
...
export function toggleSelectedGroup<T extends string | number>(
  ids: T[],
  nextIds: T[],
  replaceAll = false,
) {
  ...
  const allSelected =
    nextIds.length > 0 && nextIds.every((id) => currentSet.has(id));
  if (replaceAll) {
    return allSelected ? [] : nextIds;
  }
  if (allSelected) {
    return ids.filter((id) => !nextIds.includes(id));
  }
...
export function updateKeyedListField<
  T extends { key: string },
  K extends keyof T,
>(rows: T[], key: string, field: K, value: T[K]) {
  return rows.map((row) =>
...
export function removeKeyedListItem<T extends { key: string }>(
  rows: T[],
  key: string,
) {
...
// Draft rows need stable client-side keys until the user saves them into real
// backend records with ids of their own.
...
export function createPackageDraftRow(
  itemId = "",
  quantity = "1",
): PackageEditorRowDraft {
  return { key: nextPackageDraftRowKey(), itemId, quantity };
}
export function createEmptyInventoryDraft(
...
export function buildDonationDraft(
  ...
  return {
    ...
    receivedDate: formatUkDateInput(
      donation.pickup_date || donation.created_at,
    ),
...
export function buildLotReference(lot: {
  id: number;
...
export function buildScopedPackageRow(
  detail: FoodPackageDetailRecord,
): PackageRow {
  return {
    ...
    stock: detail.stock,
    contents: detail.package_items.map((item) => ({
      itemId: item.inventory_item_id,
      name: item.inventory_item_name,
      quantity: item.quantity,
      unit: item.inventory_item_unit,
    })),
  };
}
export function buildCodeVerifyResult(
...
type ScopedFoodBankFilterProps = Pick<
  ScopedSectionBindings,
  "foodBankOptions" | "selectedFoodBankId" | "onFoodBankChange"
>;
function buildScopedSectionBinding({
  ...
  // Keep the scope-aware search copy in one place so each section explains why
  // it is empty before a food bank has been chosen.
  return {
    ...sharedFoodBankFilterProps,
    searchDisabled,
    searchPlaceholder: searchDisabled
      ? `Choose a food bank to browse ${searchLabel}`
      : `Search ${searchLabel}`,
...
export function resolveScopedFoodBankId(
  fixedFoodBankId: number | null,
  selectedFoodBankId: number | null,
) {
  return fixedFoodBankId ?? selectedFoodBankId ?? null;
}
export function buildScopedInventoryItems(
  inventory: InventoryItem[],
  scopeState: AdminFoodBankScopeState,
) {
  // Network admins start without an effective bank scope, while local admins
  // should immediately inherit the inventory already tied to their bank.
  return scopeState.effectiveFoodBankId == null
    ? scopeState.canChooseFoodBank
      ? []
      : inventory
    : inventory.filter(
        (item) => item.foodBankId === scopeState.effectiveFoodBankId,
      );
}
export function buildScopedSectionBindings({
  ...
  const sharedFoodBankFilterProps:
    | ScopedFoodBankFilterProps
    | Record<string, never> = scopeState.canChooseFoodBank
    ? {
        foodBankOptions: foodBankFilterOptions,
        selectedFoodBankId,
        onFoodBankChange: setSelectedFoodBankId,
      }
    : {};
  return {
    inventory: buildScopedSectionBinding({
      scopeState,
      sharedFoodBankFilterProps,
      searchLabel: "inventory items",
      sectionError: availableFoodBanksError,
    }),
