...
export function makeTaskRunner({
  ...
  // Centralise the token gate so each modal and confirm flow inherits the same
  // session-expired behaviour instead of open-coding it.
  const getToken = (
    report = (message: string) => setPageNotice("error", message),
  ...
  return {
    ...
    async runBusyTask({
      ...
      try {
        ...
        reportError(toErrorMessage(error, fallbackMessage), setError, onError);
      } finally {
        setBusy(false);
      }
    },
    async runPendingTask<Result>({
      action,
      fallbackMessage,
      task,
      onError,
      ...
      fallbackMessage: string;
      task: (token: string) => Promise<Result>;
      onError?: (message: string) => void;
      onSuccess?: (result: Result) => void;
      successMessage?: string;
      afterSuccess?: () => Promise<unknown> | unknown;
      reportMissingToken?: (message: string) => void;
    }) {
      const token = getToken(reportMissingToken);
      if (!token) return undefined;
      // Pending action state is shared across the page so destructive confirms,
      // banners, and toasts can reflect cross-section work consistently.
