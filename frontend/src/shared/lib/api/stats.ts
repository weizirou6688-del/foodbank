...
export const getPublicImpact = (
  range: "month" | "quarter" | "year" = "month",
  token?: string,
) =>
  // 首页可匿名调用;管理页面传入 token 后复用同一聚合接口
  apiClient.get<PublicImpactMetricsResponse>(
    `/api/v1/stats/public-impact?range=${range}`,
    token,
  );
