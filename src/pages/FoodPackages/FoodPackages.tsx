...
export default function FoodPackages() {
  ...
  const toggleSelect = (pkgId: number) => {
    setSelections((prev) => {
      if (prev[pkgId] !== undefined) { const next = { ...prev }; delete next[pkgId]; return next }
      return { ...prev, [pkgId]: 1 }
    })
  }
  const setQty = (pkgId: number, qty: number) => setSelections((prev) => ({ ...prev, [pkgId]: qty }))
  ...
  const handleApply = () => {
    ...
    const selArray: Selection[] = Object.entries(selections).map(([id, qty]) => ({ packageId: Number(id), qty }))
    const result = applyPackages(user!.email, selArray)
    if (result.success) { setSelections({}); setCodeModal({ open: true, code: result.code! }) }
  ...
  return (
    <div className={styles.page}>
      <div className={styles.pageHeader}>
        <h1 className={styles.pageTitle}>Food Packages</h1>
        <p className={styles.pageSub}>Select the packages you need and apply. Up to 3 packages per week.</p>
      </div>
      ...
      <div className={styles.main}>
        ...
        <div className={styles.infoBar}>
          ...
          <div className={styles.weeklyBadge}>
            ...
            <span className={styles.weeklyLabel}>Remaining this week</span>
          </div>
        </div>
              ...
              >
                ...
                <div className={styles.pkgBody}>
                  ...
                  <div className={styles.stockRow}>
                    <span>Available:</span>
                    <span className={isLow ? styles.stockLow : styles.stockOk}>{pkg.stock} packs</span>
                  </div>
                  {isSelected && (
      ...
      <div className={styles.applyBar}>
        ...
        <button className={styles.applyBtn} onClick={handleApply} disabled={selectedCount === 0}>
          Apply Package →
        </button>
      </div>
      ...
      <Modal isOpen={codeModal.open} onClose={() => { setCodeModal({ open: false, code: '' }); navigate('/find-foodbank') }} title="Application Successful">
        <div className={styles.codeModalContent}>
          ...
          <p className={styles.codeHint}>Present this code at the food bank counter when collecting your package.</p>
          <button className={styles.doneBtn} onClick={() => { setCodeModal({ open: false, code: '' }); navigate('/find-foodbank') }}>
            Done
          </button>
        </div>
      </Modal>
