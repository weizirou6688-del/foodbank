...
export default function LoginModal({ isOpen, onClose, initialTab = 'signin' }: LoginModalProps) {
  ...
  const handleRegister = async () => {
    ...
    if (regForm.password.length < 6)  errors.password = 'Password must be at least 6 characters.'
          ...
          {tab === 'signin' ? (
            <>
              ...
              <div style={{ background: '#FEF9C3', border: '1px solid #F7DC6F', borderRadius: 10, padding: '0.75rem 1rem' }}>
                ...
                <p style={{ fontFamily: 'DM Sans, monospace', fontSize: '0.75rem', color: '#78350F' }}>user@example.com / user123</p>
              </div>
            </>
          ) : (
            <>
